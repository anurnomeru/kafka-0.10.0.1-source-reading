...
import org.apache.kafka.common.requests.{ControlledShutdownRequest => _, ControlledShutdownResponse => _, FetchRequest => _, FetchResponse => _, GroupCoordinatorRequest => _, GroupCoordinatorResponse => _, OffsetCommitRequest => _, OffsetCommitResponse => _, OffsetFetchRequest => _, OffsetFetchResponse => _, _}
...
class KafkaApis(val requestChannel: RequestChannel,
  ...
  def handleProducerRequest(request: RequestChannel.Request) {
    ...
    if (authorizedRequestInfo.isEmpty)
      ...
      val authorizedMessagesPerPartition: Map[TopicPartition, MessageSet] = authorizedRequestInfo.map {
